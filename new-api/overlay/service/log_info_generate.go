...
func GenerateTextOtherInfo(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelRatio, groupRatio, completionRatio float64,
	...
	// 对话留痕: 记录用户提示词与模型回复（截断，便于审计检索）
	if relayInfo.PromptText != "" {
		pt := relayInfo.PromptText
		if len(pt) > 8192 {
			pt = pt[:8192] + "...[truncated]"
		}
		other["prompt_text"] = pt
	}
	if relayInfo.ResponseText != "" {
		rt := relayInfo.ResponseText
		if len(rt) > 8192 {
			rt = rt[:8192] + "...[truncated]"
		}
		other["response_text"] = rt
	}
