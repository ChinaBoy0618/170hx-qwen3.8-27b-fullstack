...
class UnifiedTreeCore(UnifiedTreeCoreInterface):
    ...
    def _inc_hit_count_and_check(
        ...
        if node.evicted or chunked:
            return False
        # L1 bookkeeping (hit count / TTL stamp / tier2 promotion) runs on
        # every hit regardless of write policy. Under write_back the proactive
        # D->H backup below is suppressed: the node is backed up when it is
        # evicted from L1 (demotion), not on hit.
        node.hit_count += 1
        if self._ttl_stamper is not None:
