...
class Scheduler(
    ...
    def process_pending_chunked_abort(self) -> None:
        ...
        if self.enable_hicache_storage:
            self.tree_cache.release_aborted_request(req.rid)
        # Abort must not drop the prefix: register the committed (possibly
        # partial-chunk) KV into the radix before releasing, so a resend of
        # the same prompt hits the tree instead of re-prefilling from zero.
        # Mirrors the normal chunked-prefill flow (maybe_cache_unfinished_req
        # per chunk, then a finishing release with is_insert=True).
        maybe_cache_unfinished_req(req, self.tree_cache, chunked=False)
    ...
    def _maybe_hot_l3_backup(self):
        """Tick the hot L3 snapshot (write_back): persist the hottest L1-only
        set to L3 so a restart/shutdown does not lose it. Self-gated by
        interval inside tree_cache; no-op unless maybe_hot_l3_backup exists."""
        tree_cache = getattr(self, "tree_cache", None)
        fn = getattr(tree_cache, "maybe_hot_l3_backup", None)
        if fn is None:
            return
        try:
            fn()
        except Exception:
            logger.exception("hot L3 backup tick failed")
