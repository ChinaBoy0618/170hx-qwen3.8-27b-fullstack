...
class UnifiedRadixCache(BasePrefixCache):
    ...
    def _restamp_l2_lease(self, node_id: NodeId) -> None:
        """write_back demotion: a node that just moved L1→L2 gets a fresh
        lease of 2x the current L1 TTL band, so L2's TTL+hit eviction has a
        meaningful expire_at instead of inheriting a stale L1 stamp.
        (1x would understate L2's role as the persistence buffer; see plan §2.)
        """
        try:
            node = self.tree_core.node_by_id(node_id)
            node.expire_at = time.monotonic() + 2.0 * self._ttl_secs_for_usage(
                self._ttl_token_usage()
            )
        except Exception:
            pass
                ...
                while not target_reached(ct):
                    ...
                    if backup_kv is not None:
                        ...
                        if written > 0:
                            ...
                            # §2: node just moved L1→L2; give it a fresh
                            # 2x-L1-band lease so L2 TTL eviction has a
                            # meaningful expire_at (the L1 stamp is stale).
                            self._restamp_l2_lease(node_id)
    ...
    # ---- hot L3 snapshot (§1b) ----------------------------------------
    # Under write_back, data reaches L3 only at demotion time (L1 evict ->
    # D->H ack -> auto write_backup_storage). Nodes still resident on L1
    # therefore vanish on restart/shutdown. This proactively runs the same
    # D->H backup for the hottest L1-only set on a slow interval; its ack
    # auto-fires the L3 write via the existing _finish_write_through_ack
    # chain, so no new async machinery is needed. The copy + L3 write both
    # complete in the background (non-blocking for the scheduler).
    #
    # A node needs a snapshot iff it is L1-only (not `backuped`); once the
    # D->H lands the node is `backuped` and skipped forever after (a radix
    # node's content is immutable, so one snapshot is enough). Pinned nodes
    # (tier4, e.g. the CC prefix) and nodes with hit_count >= min_hits
    # (SGLANG_HOT_L3_BACKUP_MIN_HITS, default 5; 2 was too low and let
    # one-off prefixes flood L3) qualify.

    def maybe_hot_l3_backup(self) -> None:
        # v3 fix: hot L3 backup off by default. It runs D->H copies
        # synchronously in the scheduler main loop and stalled the event
        # loop 20+s under 16-concurrency (v2 zombie-card repro 2026-09-20).
        if os.environ.get("SGLANG_HOT_L3_BACKUP", "0") != "1":
            return
        if not (
            self.enable_storage
        ...
        try:
            interval = max(
                5.0, float(os.environ.get("SGLANG_HOT_L3_BACKUP_INTERVAL", "60"))
            )
        except ValueError:
            interval = 60.0
        try:
            # A prefix must be hit this many times before it's "hot" enough to
            # earn an L3 slot: with a low bar, one-off prefixes (including
            # retries) flood L3 and squeeze out genuinely hot data. Pinned
            # nodes bypass the bar (see below).
            min_hits = max(
                1, int(os.environ.get("SGLANG_HOT_L3_BACKUP_MIN_HITS", "5"))
            )
        except ValueError:
            min_hits = 5
        now = time.monotonic()
        if now - getattr(self, "_hot_l3_last_run", 0.0) < interval:
            return
        self._hot_l3_last_run = now

        # Hot set: device-resident, not yet D->H backed up, hot (tier2/pin).
        # Prune on `evicted`: a device-evicted node's subtree is also evicted
        # (eviction is leaf-up), so no hot L1-only node hides below it.
        root = self.tree_core.root_node
        candidates = []
        stack = [root]
        while stack and len(candidates) < 32:
            node = stack.pop()
            if node is root or node.evicted or node.backuped:
                # Root has no KV; evicted subtrees are fully device-free.
                stack.extend(node.children.values())
                continue
            if node.hit_count >= min_hits or getattr(node, "pinned", False):
                candidates.append(node)
            stack.extend(node.children.values())

        batch = 4  # bound D->H bursts (each allocates host RAM + mamba state)
        queued = 0
        for node in candidates:
            if queued >= batch:
                break
            if node.id in self.ongoing_write_through:
                continue  # D->H already in flight
            action = self.tree_core._build_backup_kv_action(
                node, write_back=False
            )
            self._execute_and_commit_kv_backup(action, write_back=False)
            queued += 1
        if queued:
            logger.info(
                f"hot L3 snapshot: queued D->H for {queued} hot L1-only node(s) "
                f"(ack -> L3); {len(candidates)} candidate(s) total"
            )
        ...
        while finish_count > 0:
            ...
            for ack_id in ack.node_ids:
                ...
                if self.is_write_back:
                    # §2: node just returned L2->L1; renew its lease (2x current
                    # L1 band) so a stale demotion stamp can't let the 5s TTL
                    # reclaim bounce it straight back to L2. Each load_back
                    # renews the window, so repeatedly-read-back hot prefixes
                    # keep their life extended (heat).
                    self._restamp_l2_lease(node)
