...
class LRUFileEvictor:
    ...
    def _enforce_free_space_locked(self, value_bytes: int) -> bool:
        ...
        self._evict_while(
            ...
            cap=self._eviction_cap_for(value_bytes + self.min_free_bytes),
        )
    ...
    def _evict_while(self, should_continue, cap: int = 256) -> int:
        ...
        evictions = 0
        while self._lru and attempts_left > 0 and evictions < cap and should_continue(reclaimed):
    ...
    def _evict_locked(self, needed_bytes: int) -> None:
        ...
        reclaimed = self._evict_while(
            lambda _: self._total_bytes > target,
            cap=self._eviction_cap_for(needed_bytes),
        )
        if reclaimed:
            logger.debug(
    ...
    def _eviction_cap_for(self, needed_bytes: int) -> int:
        """v6: per-call eviction budget (scales for large writes, bounded).

        Small writes keep the v5 baseline (256) so their behavior is unchanged.
        A large write (e.g. the ~78 MB mamba state block) scales the budget with
        the requested size so one reserve() can actually free the bytes it needs,
        instead of bailing after 256 tiny evictions. Hard-capped so the loop
        stays bounded and can never reproduce the v2 unbounded scan storm; the
        islice(64) window in _evict_one_lru_locked keeps each eviction O(64).
        """
        _SMALL_WRITE = 1 << 20  # 1 MiB: at/below this, keep the v5 budget
        _BASE = 256
        _HARD_MAX = 16384
        ...
        want = int(needed_bytes / avg * 1.5) + 8  # 1.5x margin + slack
