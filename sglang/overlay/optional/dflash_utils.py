...
def _dflash_is_wna16(quant_method: Any, layer: Any) -> bool:
    """True if the layer holds Marlin-packed weight-only int weights (WNA16).

    Attribute-based because in this build ``layer.quant_method`` is the base
    CompressedTensorsLinearMethod; the WNA16 *scheme* instance lives on
    ``layer.scheme`` (see CompressedTensorsLinearMethod.apply). WNA16
    create_weights registers ``weight_packed`` + ``weight_scale`` params, which
    is the definitive Marlin-packed signal.
    """
    if not (hasattr(layer, "weight_packed") and hasattr(layer, "weight_scale")):
        return False
    ...
    try:
        ...
        return True
    return isinstance(scheme, CompressedTensorsWNA16)


def dflash_dequant_kv_weight(
    qkv_proj: Any, q_size: int, kv_size: int
) -> Optional[torch.Tensor]:
    """Return the dense bf16 KV rows [2*kv_size, in_dim] of a fused QKV layer.

    Unquantized: raw row-slice view of ``.weight`` (zero copy).
    WNA16 (Marlin-packed int4/int8): a one-time identity-proxy GEMM — running
    the linear on I_in yields ``dequant(W).T``; transposed back and sliced, this
    is numerically identical to what the quantized forward produces (same GEMM
    the runtime uses) at WNA16 round-trip precision. The KV slice is cloned
    (caching only ~21MB/layer) so the transient full dense weight can be freed;
    the clone is cached on the module as ``_dflash_dense_kv_w``.
    Returns None when the scheme is unsupported (callers fall back to the
    sequential quantized forward).
    """
    cached = getattr(qkv_proj, "_dflash_dense_kv_w", None)
    ...
    if quant_method is None or isinstance(quant_method, UnquantizedLinearMethod):
        ...
        if w is None:
            return None
    elif _dflash_is_wna16(quant_method, qkv_proj):
        in_dim = int(qkv_proj.input_size)
        try:
            with torch.no_grad():
                eye = torch.eye(
                    in_dim,
                    device=qkv_proj.weight_packed.device,
            ...
            return None
        w = out.t().contiguous()
        # One-time numeric self-check: identity-proxy dequant(W) must equal the
        # quantized forward on the same input. cos~1 => dequant math correct
        # (then accept_length is a property of the draft model, not the path).
        try:
            ...
            _fwd = qkv_proj(_xs)[0]
            _a = _fwd[:, q_size : q_size + 2 * kv_size]
            _b = _xs @ w[q_size : q_size + 2 * kv_size].t()
            _lg.warning(
                "DFLASH WNA16 dequant self-check in_dim=%d cos_vs_forward=%.6f "
                "max_abs_diff=%.6f",
                in_dim,
                float(
                    torch.nn.functional.cosine_similarity(
                        _a.flatten(), _b.flatten(), dim=0
                    )
                ),
                float((_a - _b).abs().max()),
            )
            ...
            logging.getLogger(__name__).warning(
                "DFLASH WNA16 dequant self-check failed: %r", _e
            )
    else:
        return None
    if quant_method is None or isinstance(quant_method, UnquantizedLinearMethod):
        kv = w[q_size : q_size + 2 * kv_size]  # row slice of contiguous 2D = view
    else:
        kv = w[q_size : q_size + 2 * kv_size].clone()  # free full dense
    qkv_proj._dflash_dense_kv_w = kv
    return kv
...
def can_dflash_slice_qkv_weight(qkv_proj: Any) -> Tuple[bool, str]:
    ...
    Unquantized layers expose ``.weight``; WNA16 (weight-only int4/int8 Marlin)
    is supported via the identity-proxy dequant in :func:`dflash_dequant_kv_weight`.
    Other quant schemes fall back to the sequential path.
    """
    quant_method = getattr(qkv_proj, "quant_method", None)
    if quant_method is None or isinstance(quant_method, UnquantizedLinearMethod):
        ...
        return True, ""
    if _dflash_is_wna16(quant_method, qkv_proj):
        if not (
            hasattr(qkv_proj, "weight_packed")
            and hasattr(qkv_proj, "weight_scale")
        ):
            return False, "WNA16 qkv_proj is missing Marlin-packed weight tensors"
        return True, ""
    return (
        False,
