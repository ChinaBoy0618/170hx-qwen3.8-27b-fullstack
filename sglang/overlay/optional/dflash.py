...
class DFlashAttention(nn.Module):
    ...
    def kv_proj_only(
        ...
        """
        # Fast path: slice the dense KV rows of the fused QKV weight and run one GEMM.
        # dflash_dequant_kv_weight returns a raw .weight slice view for unquantized
        # layers and a cached identity-proxy dequant slice for WNA16 layers.
        can_slice_qkv_weight, _ = can_dflash_slice_qkv_weight(self.qkv_proj)
        kv_w = (
            dflash_dequant_kv_weight(self.qkv_proj, self.q_size, self.kv_size)
            if can_slice_qkv_weight
            else None
        )
        if kv_w is not None:
            bias_full = getattr(self.qkv_proj, "bias", None)
            bias = (
                bias_full[self.q_size : self.q_size + 2 * self.kv_size]
                if bias_full is not None
                else None
            )
            kv = F.linear(hidden_states, kv_w, bias)
