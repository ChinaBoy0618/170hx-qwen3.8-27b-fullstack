...
class FusedKVMaterializeHelper:
    ...
    def __init__(
        ...
        for layer_id, layer in enumerate(layers):
            ...
            kv_weight = dflash_dequant_kv_weight(
                attn.qkv_proj, attn.q_size, attn.kv_size
            )
            if kv_weight is None:
                raise RuntimeError(
                    "fused KV materialization needs a dense qkv weight but layer "
                    f"{layer_id} dequant returned None "
                    f"(quant_method={type(getattr(attn.qkv_proj, 'quant_method', None)).__name__})"
                )
