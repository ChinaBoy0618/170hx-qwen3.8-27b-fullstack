...
# ---------------------------------------------------------------------------
# SSE stream keepalive (ported from 1cat vLLM opt21)
#
# During a long prefill (e.g. cold 25K-token prompt with HiCache miss) or an
# internal stall (detokenizer hang), the engine yields nothing and the HTTP
# stream stays silent long enough for gateways/clients to time out. Emit an
# SSE comment line (": keepalive\n\n") whenever the engine stream is idle for
# more than SGLANG_STREAM_KEEPALIVE_SECS seconds (default 120, 0 disables).
# ---------------------------------------------------------------------------

_STREAM_KEEPALIVE_SENTINEL = object()


def _stream_keepalive_secs() -> float:
    try:
        return float(os.environ.get("SGLANG_STREAM_KEEPALIVE_SECS", "120"))
    except ValueError:
        return 120.0


async def _stream_keepalive_wrap(
    aiter: AsyncGenerator,
) -> AsyncGenerator:
    """Re-yield items from *aiter*; inject a sentinel on idle timeout.

    The pending __anext__ task is never cancelled on timeout (asyncio.wait
    leaves it running), so the underlying engine stream stays intact while
    keepalive markers flow to the client.
    """
    idle_secs = _stream_keepalive_secs()
    it = aiter.__aiter__()
    if idle_secs <= 0:
    ...
    try:
        while True:
            done, _ = await asyncio.wait({nxt}, timeout=idle_secs)
            if not done:
                yield _STREAM_KEEPALIVE_SENTINEL
                continue
            try:
                item = nxt.result()
...
class OpenAIServingChat(OpenAIServingBase):
    ...
    async def _generate_chat_stream(
        ...
        try:
            ...
            async for content in _stream_keepalive_wrap(
                self.tokenizer_manager.generate_request(
                    adapted_request, raw_request
                )
            ):
                if content is _STREAM_KEEPALIVE_SENTINEL:
                    yield ": keepalive\n\n"
                    continue
